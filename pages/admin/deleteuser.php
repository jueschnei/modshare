...
WHERE id=' . $uid) or error('Failed to get user info', __FILE__, __LINE__, $db->error());
...
if (strstr($_SERVER['HTTP_REFERER'], $_SERVER['REQUEST_URI'])) {
	...
	if ($user_info['status'] == 'disabledbyadmin') {
		...
		WHERE id=' . $uid) or error('Failed to delete user', __FILE__, __LINE__, $db->error());
