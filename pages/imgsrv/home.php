...
if (!file_exists(SRV_ROOT . '/data/imgsrv/' . $uid)) {
	mkdir(SRV_ROOT . '/data/imgsrv/' . $uid);
}
if (is_uploaded_file($_FILES['image']['tmp_name'])) {
	$imageExts = array('png', 'jpg', 'gif', 'bmp', 'ico');
	$ext = pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION);
	if (!in_array($ext, $imageExts)) {
		echo '<p>Invalid file extension</p>';
	} else {
		move_uploaded_file($_FILES['image']['tmp_name'], SRV_ROOT . '/data/imgsrv/' . $uid . '/' . $_FILES['image']['name']);
	}
}
...
while ($image = $db->fetch_assoc($result)) {
	echo '<tr>
		<td><a href="/imgsrv/view/' . $uid . '/' . clearHTML($image['filename']) . '">' . clearHTML($image['filename']) . '</a></td>
		<td><code>[img]http://' . $_SERVER['HTTP_HOST'] . '/imgsrv/view/' . $uid . '/' . clearHTML($image['filename']) . '[/img]</code></td>
		<td>' . format_date($image['uploaded']) . '</td>
		<th><form style="display:inline" action="/imgsrv' . (isset($_GET['uid']) ? '?uid=' . $uid : '') . '" method="post" enctype="multipart/form-data"><input type="hidden" name="delfile" value="' . clearHTML($image['filename']) . '" /><input type="submit" value="Delete" /></form></th>
	</tr>';
}
?>
</table>