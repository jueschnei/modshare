...
else if (isset($_GET['del_forum']))
{
	...
	if (isset($_POST['del_forum_comply'])) // Delete a forum with all posts
	{
		...
		$db->query('DELETE FROM '.$db->prefix.'trash_posts WHERE topic_id IN (SELECT id FROM '.$db->prefix.'trash_topics WHERE forum_id = '.$forum_id.')') or error('Unable to delete the post from trash_posts', __FILE__, __LINE__, $db->error());
		// We delete the topic from the trash_topics table
		$db->query('DELETE FROM '.$db->prefix.'trash_topics WHERE forum_id = '.$forum_id) or error('Unable to delete the topic from trash_topics', __FILE__, __LINE__, $db->error());
		/* Mod Trash Bin End */				
