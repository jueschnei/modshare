...
function strip_empty_bbcode($text)
{
	...
	// Remove empty tags
	while (!is_null($new_text = preg_replace('%\[(b|u|s|ins|del|em|i|h|colou?r|quote|img|url|email|list|topic|post|forum|user|scratchblocks)(?:\=[^\]]*)?\]\s*\[/\1\]%', '', $text)))
	{
		if ($new_text != $text)
			$text = $new_text;
		else
...
function do_bbcode($text, $is_signature = false)
{
	...
	$pattern[] = '%\[h\](.*?)\[/h\]%ms';
	$pattern[] = '%\[scratchblocks\](.*?)\[/scratchblocks\]%ms';
	...
	$replace[] = '</p><h5>$1</h5><p>';
	$replace[] = '<pre class="blocks">$1</pre>';
...
function parse_message($text, $hide_smilies)
{
	...
	//tidy stuff up for [scratchblocks]
	while (preg_match('%<pre class="blocks">(.*?)<br />(.*?)</pre>%ms', $text))
		$text = preg_replace('%<pre class="blocks">(.*?)<br />(.*?)</pre>%ms', '<pre class="blocks">$1' . "\n" . '$2</pre>', $text);
	while (preg_match('%<pre class="blocks">(.*?)&#160;(.*?)</pre>%ms', $text))
		$text = preg_replace('%\<pre class="blocks">(.*?)&#160;(.*?)</pre>%ms', '<pre class="blocks">$1$2</pre>', $text);
	while (preg_match('%<pre class="blocks">(.*?)' . "\n" . ' (.*?)</pre>%ms', $text))
		$text = preg_replace('%<pre class="blocks">(.*?)' . "\n" . ' (.*?)</pre>%ms', '<pre class="blocks">$1' . "\n" . '$2</pre>', $text);	
